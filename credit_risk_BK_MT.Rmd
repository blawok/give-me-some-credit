...
Then we have a look at how each varaible is distributed by using histograms:
...
Below plots and tables for splits are shown. We set the *smbinning* package to 
...
As we can see, there are no significant correlations between any of the variables - therefore we keep all the variables in the furhter analysis.
...
First, we estimate basic models (with Intercept only and with all variables) - these will be our benchmarks when assessing the final logit model.
...
Then, we use *stepwise (both)* method to choose only statistically significant variables. We are only providing the code, because the model is previously computed and we loaded it in the previous cell.
...
As we can see model is well fit to the data and the variables are statistically significant (based on GoF, Hosmer-Lemeshow and LR tests).
...
Now we compare ROC curves of base and max model with our stepwise model.
...
P-value of both tests is lower than the significance level (of 0.05) so we conclude that the ROC curves are not equally good.
